...
- **Stateful outer + pure inner view split** (M3-status-header exemplar): an outer `View` owns `@Environment`, `@State` models, `.task`, `.onChange`; an inner `…Content` struct takes fully-resolved value types and renders. Snapshot tests exercise the `…Content` struct directly — no environment, no fixtures, no git. Reuse for every panel-sized view (graph rows, diff viewer, toast banners). Example: `StatusHeaderView` (outer) + `StatusHeaderContent` (inner).
- **Swift actor reentrancy across `await` is NOT serializing.** A trivial `actor Q { func run(_ body:) async throws -> T { try await body() } }` mail-orders calls but interleaves their bodies across suspension points — `Task.sleep`, subprocess waits, anything that yields. `GitQueue`'s `CompletionGate` chain (each `run` awaits the previous gate, installs its own, fires it in `defer`) is the canonical remedy. Reuse the pattern whenever two callers must not interleave across `await`s.
- **`@unchecked Sendable` must carry a one-sentence justification doc-comment** explaining why the alternatives (actor, `nonisolated(unsafe)`, refactor) were rejected. Future reviewers shouldn't have to re-litigate the choice.
